...
const ListingBox = ({id,bgcolor,svg,min,max})=>{
  ...
  return (
    <div id={id} className={` ${bgcolor}  w-full col-span-12 md:col-span-4 p-5`}>
              ...
              if (index > min && index < max) {
                return (
                  <li key={index} className='text-sm text-white list-disc'>
                    <Link to={`/listings/?category=${listing.toLowerCase()}`}>{listing}</Link>
                  </li>
                )
              }
...
const HomeBanner = () => {
  ...
  return (
    <>
      <section className="w-full px-10  bg-white min-h-[600px]  relative flex flex-col justify-center items-center" style={{ background: "url(/images/banner.png)", backgroundPosition: 'center' }}>
          <div className='absolute w-full h-full bg-custom-heading-color/60 top-0 left-0 z-0' />
          
          <div className=" mx-auto flex flex-col gap-10 z-50">
            <h1 className="text-4xl md:text-65 tracking-tight font-semibold  pr-2 text-center text-white">
      ...
      <div id="popular-cats" className='w-full grid grid-cols-12 z-50'>
          {listingBoxes.map((box)=>(
            <ListingBox id={box.id} bgcolor={box.bgcolor} svg={box.svg} min={box.min} max={box.max}/>
          ))}  
      </div>
    </>
  )
