...
const CategorySearch = () => {
    ...
    const handleRemoveCat = (cat) => {
      setSelectedCats((prevSelectedCats) => prevSelectedCats.filter((item) => item !== cat));
        ...
        >
          {uniqueFranchisedCats.map((cat, index) => {
            ...
            return (
              ...
              className={`${isActive ? "border-l-4 border-custom-heading-color" : "text-black"} w-full block cursor-pointer hover:bg-white dark:hover:bg-gray-900 dark:bg-gray-800 hover:text-link px-3 py-2 `}
            >
              <span>
              {cat}
              ({listings.filter((listing) => listing.category === cat).length})
              </span>
            </div>
             {isActive &&
              <span className="text-red-950 font-bold cursor-pointer" onClick={()=>handleRemoveCat(cat)}>X</span>
            }
            </div>
            )
          })}
